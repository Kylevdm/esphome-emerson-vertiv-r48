...
/*  if (cnt == 6) {
  ...
  if (cnt == 6) {
    cnt = 0;
    // re-send the control word + sync/keepalive once per state-machine cycle
    this->set_control(this->compute_control_byte());
...
void EmersonR48Component::set_max_input_current(float value) {
    ...
    if (value < 0) {
        ESP_LOGD(TAG, "max_input_current is out of range: %f", value);
        return;
    }
...
uint8_t EmersonR48Component::compute_control_byte() {
  return this->dcOff_ << 7 | this->fanFull_ << 4 | this->flashLed_ << 3 | this->acOff_ << 2 | 1;
}
